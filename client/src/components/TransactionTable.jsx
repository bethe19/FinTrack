...
const TransactionTable = ({ transactions, darkMode }) => {
    ...
    return (
        <div className={`border ${darkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-300'}`}>
            {/* Header Controls */}
            <div className={`p-6 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${darkMode ? 'border-gray-800' : 'border-gray-300'}`}>
                <div>
                ...
                <div className="flex items-center gap-3">
                    <div className={`relative flex items-center border px-4 py-2.5 ${darkMode ? 'bg-black border-gray-800' : 'bg-white border-gray-300'}`}>
                        <Search className={`w-4 h-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'} mr-2`} />
                        <input 
                            type="text" 
                            placeholder="Search..." 
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className={`bg-transparent border-none focus:outline-none text-sm w-full sm:w-48 ${darkMode ? 'text-white placeholder-gray-500' : 'text-black placeholder-gray-400'}`}
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                    <thead className={`${darkMode ? 'bg-black border-b border-gray-800 text-white' : 'bg-white border-b border-gray-300 text-black'} font-medium uppercase text-xs tracking-wider`}>
                        <tr>
                            <th
                                className="px-6 py-4 cursor-pointer hover:opacity-70 transition-opacity select-none"
                            ...
                            <th
                                className="px-6 py-4 text-right cursor-pointer hover:opacity-70 transition-opacity select-none"
                    ...
                    <tbody className={`divide-y ${darkMode ? 'divide-gray-800' : 'divide-gray-200'}`}>
                        {paginatedTransactions.length > 0 ? (
                            paginatedTransactions.map((t) => (
                                ...
                                >
                                    <td className={`px-6 py-4 whitespace-nowrap font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className={`max-w-xs truncate font-medium ${darkMode ? 'text-gray-300' : 'text-gray-700'}`} title={t.description || 'No description'}>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`inline-flex items-center px-2.5 py-1 text-xs font-semibold border ${
                                    ...
                                    <td className={`px-6 py-4 text-right font-bold tabular-nums ${darkMode ? 'text-white' : 'text-black'}`}>
                            ...
                            <tr>
                                <td colSpan="4" className={`px-6 py-12 text-center ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
                <div className={`px-6 py-4 border-t flex items-center justify-between ${darkMode ? 'border-gray-800' : 'border-gray-300'}`}>
